# Feature 031 — Error Index Population

**Date:** 2026-03-29
**Source:** Mined from LEARNINGS.md, ANTI_PATTERNS.md, git log (all `fix:` commits), and agent memory bug records.
...
| ERR-002 | 422 Unprocessable Entity on POST /api/areas/{id}/teams/ | `TeamCreate` schema declares `functional_area_id: int` with no default; Pydantic rejects body before route handler can inject it from path | Give field a sentinel default or split it out of the schema | Feature 016 (documented bug) |
...
| ERR-007 | 500 on all endpoints when containers are healthy (`asyncpg UndefinedTableError`) | Alembic migrations never run; `make up` does not migrate automatically | Run `make migrate` after fresh start; `make reset-db` already does this | Feature 026 (investigation) |
...
| ERR-018 | Decimal conversion silently swallowed bare `except:pass` | Bare `except Exception: return` in `_append_history_if_changed` | Narrow to `except (ValueError, InvalidOperation)` | Feature 010 |
...
- Total entries: **47**
- All LEARNINGS.md "Issues caught in review" sections are covered (Phase 1–4, Feature 029)
- All 14 ANTI_PATTERNS.md entries map to at least one ERR entry
- All `fix:` prefix commits in git log are accounted for
- No `# FIXME`, `# TODO`, `# HACK`, or `# BUG` comments found in codebase
- Bug memory files (team 422, select sentinel) are included as ERR-002 and ERR-026
